...
var plugin = (() => {
  ...
  // sources/utils/functions.ts
  function getInheritanceChain(groupName) {
  ...
  async function readCatalogsYml(project) {
  ...
  async function writeCatalogsToYarnrc(project, catalogs) {
  ...
  function resolveInheritedCatalog(groupName, allGroups) {
  ...
  function resolveAllCatalogs(catalogsYml) {
    const result = {
      named: {}
    };
      ...
      if ("validation" in options && options.validation) {
        ...
        if (typeof validation === "string") {
          ...
          for (const [_, level] of Object.entries(validation)) {
            if (typeof level !== "string" || !["warn", "strict", "off"].includes(level)) {
              return false;
            }
          }
        } else {
  ...
  function validateInheritanceStructure(config) {
  ...
  var CatalogConfigurationReader = class {
    constructor() {
     ...
     */
    async readConfiguration(project) {
      ...
      if (options && catalogsYml) {
        ...
        if (!this.isValidOptions(options, catalogsYmlGroups)) {
          throw new CatalogConfigurationError(
    ...
    async shouldIgnoreWorkspace(workspace) {
      ...
      if (config.options?.ignoredWorkspaces) {
        return (0, import_picomatch.isMatch)(
          import_core.structUtils.stringifyIdent(workspace.manifest.name),
  ...
  var configReader = new CatalogConfigurationReader();
      ...
      const report = await import_core2.StreamReport.start(
        ...
        async (report2) => {
          ...
          if (this.dryRun) {
            report2.reportInfo(
              0,
              source_default.bold("Dry run mode - no files will be modified")
            );
            report2.reportSeparator();
            if (resolved.root) {
              report2.reportInfo(0, source_default.bold("Root catalog:"));
              for (const [pkg, version] of Object.entries(resolved.root)) {
                report2.reportInfo(0, `  ${source_default.cyan(pkg)}: ${version}`);
              }
              report2.reportSeparator();
            }
            if (namedCount > 0) {
              report2.reportInfo(0, source_default.bold("Named catalogs:"));
              for (const [groupName, group] of Object.entries(resolved.named)) {
                report2.reportInfo(0, source_default.yellow(`  ${groupName}:`));
  ...
  async function getPackageVaidationLevel(workspace, packageName) {
